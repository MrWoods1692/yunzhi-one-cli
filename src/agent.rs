...
impl<C: LlmClient> Agent<C> {
	pub fn new(client: C, cwd: PathBuf, options: AgentOptions, prompter: Arc<dyn PermissionPrompter>) -> Result<Self> {
	...
	pub fn estimated_tokens(&self) -> usize {
		estimate_tokens(&self.system_prompt) + self.history.iter().map(|message| estimate_tokens(&message.text())).sum::<usize>()
	}
	...
	pub async fn run_turn(&mut self, input: String) -> Result<String> {
		...
		loop {
				...
				match event? {
					...
					StreamEvent::ToolUse(call) => {
						if !text_accumulator.is_empty() {
							assistant_blocks.push(ContentBlock::text(std::mem::take(&mut text_accumulator)));
						}
			...
			for call in tool_calls {
				...
				let output = self.tools.execute(&call.name, call.input, &mut self.context).await;
				tui::print_tool_done(!output.is_error, tool_started.elapsed().as_secs_f32());
				self.history.push(Message::tool_result(call.id, output.content, output.is_error));
			}
			self.compress_if_needed();
		}
	}
	...
	fn compress_if_needed(&mut self) {
		...
		let summary = old
			...
			.collect::<Vec<_>>()
			.join("\n");
		self.history.insert(0, Message::system(format!("早期对话摘要（字符级压缩）:\n{}", truncate_chars(&summary, 4000))));
	}
}
...
impl PermissionPrompter for DenyPrompter {
	async fn confirm(&self, _request: PermissionRequest) -> Result<crate::tools::PermissionDecision> {
